...
contract kStakingVaultModulesTest is DeploymentBaseTest {
    ...
    /// @dev Test deployBatchReceiver through interface
    function test_DeployBatchReceiver_Interface() public {
        // Non-asset router should fail with access control
        vm.prank(users.alice);
        vm.expectRevert(BaseModule.OnlyKAssetRouter.selector);
        vaultInterface.deployBatchReceiver(TEST_BATCH_ID);
    }

    /// @dev Test deployBatchReceiver with asset router
    function test_DeployBatchReceiver_AssetRouter() public {
        vm.prank(address(assetRouter));
        address receiver = vaultInterface.deployBatchReceiver(TEST_BATCH_ID);

        assertTrue(receiver != address(0), "Should deploy batch receiver");

        // Should be idempotent
        vm.prank(address(assetRouter));
        address receiver2 = vaultInterface.deployBatchReceiver(TEST_BATCH_ID);
    ...
    function test_DeployBatchReceiver_AccessControl() public {
        ...
        for (uint256 i = 0; i < nonRouters.length; i++) {
            ...
            vm.expectRevert(BaseModule.OnlyKAssetRouter.selector);
            vaultInterface.deployBatchReceiver(1);
        }
    }
    ...
    function test_ModulesWithExistingFunctions() public {
        ...
        vm.prank(address(assetRouter));
        address receiver = vaultInterface.deployBatchReceiver(1);
    ...
    function test_InterfaceFunctionSignatures() public {
        ...
        // Test deployBatchReceiver (takes uint256, returns address)
        vm.prank(address(assetRouter));
        address result = vaultInterface.deployBatchReceiver(1);
    ...
    function test_InterfaceErrorHandling() public {
        ...
        vm.expectRevert(BaseModule.OnlyKAssetRouter.selector);
        vaultInterface.deployBatchReceiver(999);
    }
    ...
    function test_EdgeCaseInputs() public {
        ...
        // Test deployBatchReceiver with uint32 max (safe casting)
        vm.prank(address(assetRouter));
        address receiver = vaultInterface.deployBatchReceiver(type(uint32).max);
    ...
    function test_ZeroValueInputs() public {
        ...
        vm.prank(address(assetRouter));
        address receiver = vaultInterface.deployBatchReceiver(0);
    ...
    function test_CompleteModuleIntegration() public {
        ...
        vm.prank(users.alice);
        vm.expectRevert(BaseModule.OnlyKAssetRouter.selector);
        vaultInterface.deployBatchReceiver(2);
