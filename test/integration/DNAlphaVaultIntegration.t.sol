...
contract DNAlphaVaultIntegrationTest is IntegrationBaseTest {
    ...
    function test_PegProtectionDNPullsFromAlpha() public {
        ...
        assertTrue(
            dnBalanceAfterProtection >= redemptionAmount, "DN vault should have sufficient assets after peg protection"
        );
    ...
    function test_HighFrequencyRebalancing() public {
        ...
        for (uint256 i = 0; i < numRebalances; i++) {
            if (i % 2 == 0) {
                // DN to Alpha
                executeVaultTransfer(address(dnVault), address(alphaVault), rebalanceAmount, getCurrentDNBatchId() + i);
            } else {
                // Alpha to DN
