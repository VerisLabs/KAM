...
contract MultiFacetProxy is Proxy, OwnableRoles {
    ...
    function _implementation() internal view override returns (address) {
        ...
        if (implementation == address(0)) {
            ...
            revert("No implementation for selector");
        }
        return implementation;
    }

    /// @notice Override fallback to only handle mapped selectors
    /// @dev Only delegates to modules if selector is explicitly mapped
