...
contract CustodialAdapter is BaseAdapter, Initializable, UUPSUpgradeable {
    ...
    function _getCustodialAdapterStorage()
        internal
        pure
        returns (CustodialAdapterStorage storage $)
    {
        assembly {
    ...
    function deposit(
        ...
        uint256 amount,
        address onBehalfOf
    ) external nonReentrant onlyKAssetRouter whenRegistered {
    ...
    function redeem(
        ...
        uint256 amount,
        address onBehalfOf
    ) external virtual nonReentrant onlyKAssetRouter whenRegistered {
        if (asset == address(0)) revert InvalidAsset();
        if (amount == 0) revert InvalidAmount();
        if (onBehalfOf == address(0)) revert InvalidAsset();
    ...
    ) internal view override onlyRoles(ADMIN_ROLE) {
